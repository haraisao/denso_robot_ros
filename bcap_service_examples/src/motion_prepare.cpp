...
#if 1
  Args.vt = VT_EMPTY;
  hr = bCap_RobotExecute(m_fd, lhRobot, L"GetMotionPreparationState", Args, &lResult);

  if (!lResult.boolVal){
  
    Args.vt = VT_EMPTY;
    hr = bCap_RobotExecute(m_fd, lhRobot, L"ManualResetPreparation", Args, &lResult);
   

    Args.vt = VT_EMPTY;
    hr = bCap_RobotExecute(m_fd, lhRobot, L"MotionPreparation", Args, &lResult);
